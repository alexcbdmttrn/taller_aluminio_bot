...
async def _resolver_proyecto_o_pedir(
    ...
    if nombre_corto:
        ...
        if len(coincidencias) > 1:
            ...
            coincidencias = [
                p for p in proyectos if nombre_corto.lower() in (p["descripcion"] or "").lower()
            ]
            if len(coincidencias) == 1:
                return coincidencias[0], None
            if len(coincidencias) > 1:
...
async def tool_registrar_proyecto(
    ...
    result = await ejecutar_query(
        insert, (cliente_id, nombre_corto or "Proyecto General", descripcion, monto, notas), fetch=True
    )
    return {
        "exito": True,
...
async def tool_registrar_compra_material(
    ...
    await ejecutar_query("""
        ...
        SET material_comprado = TRUE,
            ...
            costo_material = COALESCE($1, costo_material)
        WHERE id = $2
    """, (costo, pid))
    msg = f"Material marcado como comprado para '{nc}' de {cliente}."
    if costo:
        msg += f" Costo: ${costo:.2f}."
...
async def tool_consultar_gastos(limite: int = 10):
    ...
    data = [
        {"fecha": r["fecha"].strftime("%d/%m %H:%M"), "descripcion": r["descripcion"], "monto": float(r["monto"])}
        for r in resultados
    ]
...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Marca un proyecto como presupuesto enviado. NO retrocede el estado si el proyecto ya avanzó.",
            "parameters": {
                "type": "object",
                "properties": {
                    "cliente": {"type": "string"},
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Programa un nuevo recordatorio. Interpreta fechas informales y pide confirmación si hay ambigüedad.",
            "parameters": {
                "type": "object",
                "properties": {
                    "mensaje": {"type": "string"},
...
TOOL_FUNCTIONS = {
    ...
    "tool_registrar_compra_material": tool_registrar_compra_material,
...
SYSTEM_PROMPT_BASE = (
    ...
    "No asumas información. Si el usuario escribe algo confuso, PREGÚNTALE para aclarar antes de ejecutar cualquier acción. "
    ...
    "Cuando el usuario pida un recordatorio, interpreta la fecha y hora que menciona y llama a tool_crear_recordatorio directamente. "
    "La herramienta ya se encarga de pedir confirmación si hay ambigüedad AM/PM o si la hora ya pasó — no le preguntes tú también en lenguaje natural antes de llamarla. "
    "REGLA PARA MATERIAL COMPRADO: "
        ...
        for _ in range(max_iteraciones):
            try:
                ...
                if "maximum context length" in error_msg.lower() or "token" in error_msg.lower():
                    ...
                    await update.message.reply_text("⚠️ Detecté un error en mi memoria y la limpié automáticamente. Por favor, repite tu última petición, jefe.")
                else:
...
async def manejar_confirmacion(update: Update, context: ContextTypes.DEFAULT_TYPE, texto: str):
    try:
        ...
        afirmativas = {"sí", "si", "s", "yes", "y", "dale", "ok", "okay", "confirmo", "confirmar", "confirmado", "adelante", "programa", "programar"}
